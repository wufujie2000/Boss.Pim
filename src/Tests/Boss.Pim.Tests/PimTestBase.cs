...
namespace Boss.Pim.Tests
{
    public abstract class PimTestBase : AbpIntegratedTestBase<PimTestModule>
    {
        ...
        protected void UsingDbContext(Action<PimDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PimDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PimDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PimDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PimDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PimDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PimDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PimDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PimDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PimDbContext, Task<T>> func)
        {
            T result;
