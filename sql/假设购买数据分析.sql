...
(
    SELECT fun.TypeName 基金分类,
           fun.ShortName 基金名称,
           fun.Code 基金编码,
           CONVERT(VARCHAR(32), val.Date, 20) 时间,
    ...
    FROM dbo.FundCenter_Funds fun
        INNER JOIN dbo.FundCenter_NetWorths val
            ON val.FundCode = fun.Code
               AND DATEDIFF(DAY, @NetWorthDate, val.Date) = 0
               AND ISNULL(val.UnitNetWorth, 0) > 0
        LEFT JOIN #dtGreat
...
IF OBJECT_ID('tempdb..#dtActual') IS NOT NULL
    DROP TABLE #dtActual;
SELECT *
INTO #dtActual
FROM
(
    SELECT ROUND((sell.UnitNetWorth / buy.UnitNetWorth - 1 - 0.01) * 100, 4) 实涨幅,
           gue.*
    FROM #dtGuess gue
