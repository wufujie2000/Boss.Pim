...
SELECT TOP 2000
    CONVERT(VARCHAR(32), val.Date, 20) 估值时间,
    fun.Code 基金编码,
    val.DailyGrowthRate 估值涨幅,
    ROUND((1 / val.UnitNetWorth - 1 / #dtGreat.GreatSale) * 100, 3) 收益和,
    ROUND((1 / val.UnitNetWorth - 1 / #dtGreat.GreatBuy), 3) 建议和,
    ...
    ROUND((1 / val.UnitNetWorth - 1 / gue10.GreatSale) * 100, 3) 估10天收益比,
    ROUND((1 / val.UnitNetWorth - 1 / gue20.GreatSale) * 100, 3) 估20天收益比,
    ROUND((1 / val.UnitNetWorth - 1 / gue30.GreatSale) * 100, 3) 估30天收益比,
    ROUND((1 / val.UnitNetWorth - 1 / gue40.GreatSale) * 100, 3) 估40天收益比,
    ROUND((1 / val.UnitNetWorth - 1 / gue50.GreatSale) * 100, 3) 估50天收益比,
    ROUND((1 / val.UnitNetWorth - 1 / gue60.GreatSale) * 100, 3) 估60天收益比,
    ROUND((1 / val.UnitNetWorth - 1 / gue10.GreatBuy), 3) 估10天建议,
    ROUND((1 / val.UnitNetWorth - 1 / gue20.GreatBuy), 3) 估20天建议,
    ROUND((1 / val.UnitNetWorth - 1 / gue30.GreatBuy), 3) 估30天建议,
    ROUND((1 / val.UnitNetWorth - 1 / gue40.GreatBuy), 3) 估40天建议,
    ROUND((1 / val.UnitNetWorth - 1 / gue50.GreatBuy), 3) 估50天建议,
    ROUND((1 / val.UnitNetWorth - 1 / gue60.GreatBuy), 3) 估60天建议,
    ...
    fun.DkhsCode
FROM dbo.FundCenter_Funds fun
    LEFT JOIN dbo.FundCenter_NetWorths val
        ON val.FundCode = fun.Code
           AND DATEDIFF(DAY, val.Date, @PeriodStartDate) = 0
    LEFT JOIN #dtGreat
